...
class JXTextField(text: String? = null, columns: Int = 0) : JBTextField(text, columns.coerceAtLeast(0)) {
    ...
    private val changeListeners: MutableList<(JXTextField, String) -> Unit>? = mutableListOf()

    inline fun addTextChangeListener(crossinline listener: (String) -> Unit) {
        addTextChangeListener { _, str -> listener(str) }
    }

    fun addTextChangeListener(listener: (JXTextField, String) -> Unit) {
        changeListeners?.add(listener)
    }

    fun removeTextChangeListener(listener: (JXTextField, String) -> Unit) {
